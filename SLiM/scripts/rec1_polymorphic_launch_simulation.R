...
slimoutpath = paste0(generalpath, "polymorphic_outputs/rawdata/test/") # Where to save 
slimfile = "rec1_polymorphic_simulation.slim" # The main slim file (NOTE: there is a second annex slim file "genomatrixToSlim.slim")
...
paramdf.polym = function(N=1000,fit.vars=0.01,nloci=100, 
                         modifier=0, recpos= 717890, selfing=0,
                         beta=0){
  ...
  #recpos = Position of rec-1 in bp
  ...
  # modifier =  If 0: rec-1 WT vs Mut; If 1: WT vs Low rr modifier (25cM; codominant; WT rr landscape)
  
  
  parameters = setNames(expand.grid(list(nloci, fit.vars, N, beta, selfing, modifier,recpos)),
                        c('lociperchr', 'fitvar','popsize','beta','selfrate', 'modifier', 'recpos'))
...
simulation_polym = function(ngmatrix,nrunpergmatrix, parameters, slimpath, slimfile, transitpath, cemeesnps, smoothmaps, ngeneration=100, 
                            Nbase=1000,burn.fitvar=0.15,recchrom="I",burn=T, burntype=c("stab", "drift"), outputMSgenerations = NULL, ncores=3, dominance = F){
  ...
  lapply(1:nrow(gparam), function(i){
    ...
    paramx = parameters[parameters$recpos==recpos & parameters$lociperchr==lociperchr, ] 
    ...
    info = INITIAL_POPS(ngmatrix=ngmatrix, #number of initial population (~genotype matrix) for a set of gmatrix param (nlociperchrom, recpos)
                        ...
                        recchrom=recchrom, # The chromosome containing rec-1
    ...
    mclapply(1:nrow(paramx),mc.cores = ncores, function(p){
      ...
      fitnessout = EVOLUTION_POLYM(gid = paramx$gid[p], # The identifier of the initial pop
                        slimpath=slimpath, # The path to the directory containing the slim scripts
                        ...
                        reclocus = paramx$reclocus[p], # The position of the rec-1 locus (in number of loci)
                        ...
                        HighAndLowMod=paramx$modifier[p], # If 0: rec-1 WT vs Mut; If 1: WT vs Low rr modifier (25cM; codominant; WT rr landscape)
      ...
      fitnessout$recpos = paramx$recpos[p]
      
      gid=fitnessout$gid[1]
      
      
      ### Write the .txt file with fitness
      write.table(fitnessout, file = paste0(slimoutpath, "selectionmodifier_",ifelse(paramx$modifier[p]==1, 'low', 'rec1'),
      ...
      diffsel = stat.poly(loci=loci, transitpath=transithpath, fitnessout=fitnessout, burn = burn)
      ...
      write.table(diffsel, file = paste0(slimoutpath, "deltaLogFitness_",ifelse(paramx$modifier[p]==1, 'low', 'rec1'),
                                            "_pos", paramx$recpos[p],
                                            "_nloci", paramx$lociperchr[p],"_fitvar", paramx$fitvar[p],
                                            "_selfing", paramx$selfrate[p],
                                            "_N", paramx$popsize[p], '_',gid,".txt"), 
